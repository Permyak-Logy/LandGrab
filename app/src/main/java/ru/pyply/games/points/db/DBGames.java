...
public class DBGames {
    ...
    private static final int GAMES_NUM_COLUMN_START = 2;
    private static final int GAMES_NUM_COLUMN_END = 3;
    private static final int GAMES_NUM_COLUMN_TARGET_POINTS = 4;
    private static final int GAMES_NUM_COLUMN_LIMIT_TIME = 5;
    ...
    private static final int RESULTS_NUM_COLUMN_PLAYER_ID = 0;
    private static final int RESULTS_NUM_COLUMN_GAME_ID = 1;
    private static final int RESULTS_NUM_COLUMN_RESULT = 2;
    private static final int RESULTS_NUM_COLUMN_M_POINTS = 3;
    private static final int RESULTS_NUM_COLUMN_CAPTURED_POINTS = 4;
    private static final int RESULTS_NUM_COLUMN_LOST_POINTS = 5;
    private static final int RESULTS_NUM_COLUMN_COUNT_MOVES = 6;
    private static final int RESULTS_NUM_COLUMN_TOTAL_TIME_MOVES = 7;
    ...
    public long insertResult(long player_id, long game_id, int result, int m_points, int captured_points, int lost_points, int count_moves, long total_time_moves) {
    ...
    public long insertResult(Result result) {
        return insertResult(result.player_id, result.game_id, result.result, result.m_points, result.captured_points, result.lost_points, result.count_moves, result.total_time_moves);
    }
    ...
    public void deleteResult(long player_id, long game_id) {
    ...
    public Result selectResult(long player_id, long game_id) {
        @SuppressLint("Recycle") Cursor mCursor = database.query(TABLE_RESULTS, null, RESULTS_COLUMN_PLAYER_ID + " = ? AND " + RESULTS_COLUMN_GAME_ID + " = ?",
    ...
    public ArrayList<Result> selectAllResults() {
        @SuppressLint("Recycle") Cursor mCursor = database.query(TABLE_RESULTS, null, null, null, null, null, null);

        ArrayList<Result> arr = new ArrayList<Result>();
        mCursor.moveToFirst();
        if (!mCursor.isAfterLast()) {
            do {
                long player_id = mCursor.getLong(RESULTS_NUM_COLUMN_PLAYER_ID);
                long game_id = mCursor.getLong(RESULTS_NUM_COLUMN_GAME_ID);
                int result = mCursor.getInt(RESULTS_NUM_COLUMN_RESULT);
                int m_points = mCursor.getInt(RESULTS_NUM_COLUMN_M_POINTS);
                int captured_points = mCursor.getInt(RESULTS_NUM_COLUMN_CAPTURED_POINTS);
                int lost_points = mCursor.getInt(RESULTS_NUM_COLUMN_LOST_POINTS);
    ...
    public int updatePlayer(Player player) {
        ContentValues cv = new ContentValues();
        cv.put(PLAYERS_COLUMN_ID, player.id);
    ...
    public Player selectPlayer(long id) {
        @SuppressLint("Recycle") Cursor mCursor = database.query(TABLE_PlAYERS, null, PLAYERS_COLUMN_ID + " = ?",
    ...
    public ArrayList<Player> selectAllPlayers() {
        @SuppressLint("Recycle") Cursor mCursor = database.query(TABLE_PlAYERS, null, null, null, null, null, null);

        ArrayList<Player> arr = new ArrayList<Player>();
        mCursor.moveToFirst();
        if (!mCursor.isAfterLast()) {
            do {
                long id = mCursor.getLong(PLAYERS_NUM_COLUMN_ID);
                String name = mCursor.getString(PLAYERS_NUM_COLUMN_NAME);
    ...
    private static class OpenHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            db.execSQL("CREATE TABLE " + TABLE_PlAYERS + " (" +
                    PLAYERS_COLUMN_ID + " LONG PRIMARY KEY AUTOINCREMENT NOT NULL," +
                    PLAYERS_COLUMN_NAME + " STRING UNIQUE NOT NULL" +
                    ");"
            );

            db.execSQL("CREATE TABLE " + TABLE_GAMES + " (" +
                    GAMES_COLUMN_ID + " LONG PRIMARY KEY AUTOINCREMENT," +
            ...
            db.execSQL("CREATE TABLE " + TABLE_RESULTS + " (" +
                    RESULTS_COLUMN_PLAYER_ID + " LONG REFERENCES "+ TABLE_PlAYERS + " (" + PLAYERS_COLUMN_ID + ") PRIMARY KEY, " +
                    RESULTS_COLUMN_GAME_ID + " LONG REFERENCES " + TABLE_GAMES + " (" + GAMES_COLUMN_ID + ") PRIMARY KEY," +
